...
세션은 화면의 `중지`를 누르거나 영상이 끝날 때까지 계속됩니다. 화면에는 가장 최근 확정 자막과 진행 중 자막을 합쳐 정확히 2줄만 시간순으로 표시하지만, 이는 UI의 rolling projection일 뿐입니다. WebSocket API는 세션 동안 partial/final 이벤트를 계속 내보내므로 운영 클라이언트는 필요한 기록 정책을 별도로 적용할 수 있습니다.

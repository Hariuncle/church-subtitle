...
제어 화면의 `운영 모드` 선택으로 세 가지 방식 중 하나를 고릅니다. 기본값은 `하이브리드`이며, 세션 실행 중에는 모드를 바꿀 수 없습니다(먼저 `중지`).

- **수동**: AI 전사를 사용하지 않습니다. `시작` 버튼과 지연 선택이 비활성화되고 OpenAI 비용도 발생하지 않으며, 운영자 데스크의 수동 자막 입력과 줄 수정만으로 송출합니다. 레거시 자막기와 같은 순수 타이핑 운영입니다.
- **AI 전용**: AI 자막을 그대로 송출합니다. 운영자 데스크가 숨겨져 줄 수정·수동 삽입이 차단됩니다. `config/replacements.txt` 용어 치환은 자동 파이프라인의 일부이므로 이 모드에서도 계속 적용됩니다.
- **하이브리드**: AI 자막 송출과 운영자 데스크(줄 수정·수동 삽입)를 함께 사용합니다.

모드는 화면 투영 계층의 설정이며 서버·WebSocket API 동작을 바꾸지 않습니다.

### 자막 수정과 수동 개입 (운영자 데스크)

제어 화면 하단의 `자막 수정` 패널은 AI 자막을 송출한 뒤 고치는 선송출-후수정 방식입니다.

- **확정 줄 수정**: 최근 확정 자막이 최대 8줄 표시됩니다. 줄을 클릭해 내용을 고치고 `확정`(Enter)하면 화면 문단과 송출 화면에 즉시 반영됩니다. `취소`(Esc)는 버립니다. 운영자가 수정한 줄은 잠기며, 이후 AI가 같은 줄을 다시 보내도 무시됩니다. 내용을 비우고 확정하면 그 줄이 삭제되고 AI 재등장도 차단됩니다.
- **수동 자막**: 항상 활성인 입력창에 자막을 입력하고 Enter를 누르면 자막 흐름 끝에 삽입됩니다. 전사 세션이 없거나 AI 연결이 끊긴 뒤에도 동작하므로, 장애 시 별도 프로그램 없이 같은 송출 화면에서 레거시식 수동 타이핑으로 즉시 전환할 수 있습니다.
- **용어 치환 사전**: `config/replacements.txt`에 `잘못된 표기 => 올바른 표기`를 한 줄씩 적으면(`#`는 주석) AI 자막이 화면에 표시되기 직전에 치환됩니다. 위치별 최장 일치를 한 번만 적용하며, 운영자가 입력하거나 수정한 줄에는 적용하지 않습니다. 파일 수정 후 제어 화면을 새로고침하면 반영됩니다(서버 재시작 불필요).

수정·삽입·치환은 모두 화면 투영 계층의 기능입니다. WebSocket API가 내보내는 CaptionUpdate 이벤트와 `runs/` 기록물은 원문 그대로 유지됩니다.
